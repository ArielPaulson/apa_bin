...
#################### FIXME: DON'T KNOW IF BRANCHES OF DECISION TREE BELOW PRODUCE EQUIVALENT PRODUCTS
    ...
    if (length(wo)>0) {
        ...
        if (!file.exists(f3)) write.fasta(c(REF=paste(pa.ref[pa.ref!="-"],collapse=""), PDB=pdb.exp2), f3)
        ##pa.aln <- pa.aln2 <- t(muscle(f3, f4, as.matrix=TRUE, fix.terminals="AA"))
        if (file.exists(f4)) {
            pa.aln <- do.call(cbind, strsplit(read.fasta(f4),""))
        } else {
            pa.aln <- t(muscle(f3, f4, as.matrix=TRUE, fix.terminals="AA"))
        }
        pa.aln2 <- pa.aln
