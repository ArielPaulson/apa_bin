...
## README:
## To make non-generic, edit all <EDITME_XXXX> tags in the file appropriately.
## There are 3 tag types needing 3 values: 
## <EDITME_ORG> indicates default blast db prefix (should be organism species or genus_species) 
## <EDITME_SCRIPT> is the name you will give this script
## <EDITME_DBPATH> is the full rooted path to the blast db(s) for this script.  Should be same dir as for this script.
## Then delete this README block.
...
are de novo RNAseq assemblies; inputs are gene fastas from other species.

perl <EDITME_SCRIPT>.pl [-i input] [-s species] [-p program] [-c cores] [-o output_folder] [-mx min_subj_sim] [-mc min_subj_covg] [-mi min_subj_ident] [-ml min_align_length] --multi --nomap --noshort
...
Species prefix to blast against; default '<EDITME_ORG>'
...
perl <EDITME_SCRIPT>.pl -i genes.fa -s <EDITME_ORG> -p blastn

=item B<Protein blast, minimum query coverage 50%, minimum query identity 50%>

perl <EDITME_SCRIPT>.pl -i genes.fa -s <EDITME_ORG> -p blastp -mc 0.5 -mi 0.5

=item B<Blastx, minimum alignment length 1kb, no map images>

perl <EDITME_SCRIPT>.pl -i genes.fa -s <EDITME_ORG> -p blastx -ml 1000 --nomaps


=head1 OUTPUTS

=item B<the input fasta>

A copy of the query fasta 
...
The blast map images, one per query, showing alignments of contigs
...
my %ndb = map {($_=>1)} qw/ blastn tblastn /;  # those using nt database
my %pdb = map {($_=>1)} qw/ blastp blastx /;  # those using aa database

# add new species here
my %knownspecies = map {($_=>1)} qw/ <EDITME_ORG> /;  # list of 'species' or 'genus_species' or whatever blast-db prefixes exist
my $path = '<EDITME_DBPATH>';  # full path dir folder containing the blast databases for this script

## get command line
my ($help, $man);
my ($prog, $input, $minsim, $mincov, $minidt, $minaln, $multi, $nomap);
my ($species, $cores, $outdir) = ('<EDITME_ORG>', 2, 'results');           # defaults
GetOptions(
    "s=s" => \$species,      # 'species' or 'genus_species' or other blast-db prefix
...
while (<KEY>) {
    ...
    $_ =~ s/[\n\r]+$//;
    my ($header, $length, $status, @frames) = split /\t/, $_;  # $status doesn't do anything yet, but the file must have this format
    if ($prog eq 'blastn') {
	    ...
	    my $header2 = $pdb{$prog} ? "$header|$frame" : $header;
...
##my %stathash = ("CODING" => 0, "BORDERLINE" => 1, "NONCODING" => 2);   # available, but not implemented: can convert a contig status into a plot feature, like contig hit shading or something
my %YN = (1 => 'Y', 0 => 'N');
...
while (<IN>) {
    ...
    $scored{$query}{$subject}{$score}{"$qname\t$line2\t$morefields"} = [$qstart, $qend, (join "\t", ($qname, $qrylen, $qstart, $qend, $subjlen, $sstart, $send, $idtdec, 'NA'))];  # 'NA' can be replaced with contig status value from %stathash
    $linecount++;
}
close IN;
...
my ($prebest, $best);
open OUT, "> $blastfilt";
print OUT "Query\tSubject\tIdentity%\tAlignLen\tMismatches\tGaps\tQueryStart\tQueryEnd\tSubjStart\tSubjEnd\tE-Val\tScore\tQueryLen\tQueryCov%\tSubjLen\tSubjCov%\tSubjSim%\tSubjStatus";
print OUT "\tORFLen\tMaxOrf?" unless $prog eq 'blastn';
print OUT "\n";
unless ($nomap) {
    open MAP, "> $mapdata";
...
foreach my $query (sort keys %scored) {
    my (@order, @starts, @ends, @lines, @maplines);
	...
	my %staging;
		...
		foreach my $line (keys %{ $scored{$query}{$subject}{$score} }) {
		    my ($start, $end, $mapline) = @{ $scored{$query}{$subject}{$score}{$line} };
		    push @starts, $start;
		    push @ends, $end;
		    push @lines, $line;
		    push @maplines, $mapline;
		    $best++;
		}
            }
        } else {
		...
		my ($start, $end, $mapline) = @{ $scored{$query}{$subject}{$topscore}{$line} };
		push @starts, $start;
		push @ends, $end;
		push @lines, $line;
		push @maplines, $mapline;
    ...
    @order = sort { $starts[$a] <=> $starts[$b] or $ends[$b] <=> $ends[$a] } (0..$#starts);   # sort by start increasing then by end decreasing, which is map plot order
    foreach (@order) {
	print OUT "$lines[$_]\n";
	print MAP "$maplines[$_]\n" unless $nomap;
    }
}
close OUT;
...
for (i in 1:nrow(qry)) {
     ...
     x <- x[length(x):1]  # plot "in reverse"
     ht <- length(x) + 1; ht
     wd <- qry[i,2]; wd
     imght <- 300 + (ht - 10) * 4
     yht <- ifelse(ht < 10, 10, ht)
     yax <- if (ht > 100) { 
	       round(seq(1, length(x), length.out=20),0)
            } else if (ht > 10) {
	       round(seq(1, length(x), length.out=10),0)
            } else {
	       1:ht
	    }
     message(i, ht, imght)
